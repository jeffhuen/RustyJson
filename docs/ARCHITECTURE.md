...
3. **Custom encoding**: Supports `@derive RustyJson.Encoder` for struct field filtering and targeted ecosystem encoders such as `Phoenix.LiveView.JS`. RustyJson does not generally fall back to `Jason.Encoder` or `JSON.Encoder` for arbitrary structs.
